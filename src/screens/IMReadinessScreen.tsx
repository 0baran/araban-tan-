...
export default function IMReadinessScreen({onBack}: Props) {
  ...
  const allPassed = availableTests.length > 0 && availableTests.every(t => t.completed);
  ...
  return (
    <SafeAreaView edges={['top', 'bottom', 'left', 'right']} style={[styles.container, {backgroundColor: colors.bg}]}>
      ...
      {loading ? (
        <ActivityIndicator color="#00bfff" style={{marginTop: 60}} size="large" />
      ) : (
        <ScrollView contentContainerStyle={{paddingBottom: 20}}>
          {/* Ozet Kartı */}
          <View style={[styles.glassCard, {backgroundColor: colors.card, borderColor: colors.cardBorder}]}>
            <Text style={[styles.cardLabel, {color: colors.textMuted}]}>GENEL DURUM</Text>
            <View style={[styles.summaryBox, {backgroundColor: allPassed ? 'rgba(0,255,127,0.1)' : 'rgba(255,71,87,0.1)'}]}>
              <Text style={[styles.summaryText, {color: allPassed ? '#00ff7f' : '#ff4757'}]}>
            ...
            <View style={styles.dtcRow}>
              <View style={styles.dtcItem}>
                ...
                <Text style={[styles.dtcLabel, {color: colors.textMuted}]}>HATA KODU</Text>
              </View>
              <View style={styles.dtcItem}>
                <Text style={[styles.dtcValue, {color: data?.milOn ? '#ff4757' : '#00ff7f'}]}>
                  {data?.milOn ? 'AÇIK' : 'KAPALI'}
                </Text>
                <Text style={[styles.dtcLabel, {color: colors.textMuted}]}>MOTOR IŞIĞI</Text>
              </View>
            </View>
          </View>
          ...
          <View style={[styles.glassCard, {backgroundColor: colors.card, borderColor: colors.cardBorder}]}>
            ...
            {availableTests.length === 0 ? (
              <Text style={[styles.emptyText, {color: colors.textMuted}]}>Test verisi okunamadı.</Text>
            ) : (
              availableTests.map((test, index) => (
                <View key={index} style={[styles.testRow, {borderBottomColor: colors.cardBorder}]}>
                  <Text style={[styles.testName, {color: colors.text}]}>{test.name}</Text>
                  <View style={[styles.statusBadge, {backgroundColor: test.completed ? 'rgba(0,255,127,0.1)' : 'rgba(255,165,0,0.1)'}]}>
                    <Text style={[styles.statusText, {color: test.completed ? '#00ff7f' : '#ffa502'}]}>
...
const styles = StyleSheet.create({
  ...
  summaryText: {
    fontSize: 18, fontWeight: '900', marginBottom: 6,
  },
  summaryDesc: {
  ...
  emptyText: {
    textAlign: 'center', fontSize: 13, marginVertical: 12,
  },
});
