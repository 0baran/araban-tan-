...
import {
  ...
  TextInput, AppState, StatusBar, Permission, Linking,
} from 'react-native';
...
import {loadVehicles, getVehicles, getActiveVehicleId, Vehicle} from '../services/VehicleStorage';
import {getDTCScanCounts} from '../services/DTCHistory';
import {DTC_DESCRIPTIONS, getDTCCategory, getDTCCategoryColor, DTC_AI_ADVICE} from '../services/DTCDatabase';
...
import {checkForUpdate, promptUpdate, downloadActive, downloadProgress, onDownloadProgress} from '../services/UpdateService';
import {setupUpdateChannel, handleNotificationPress} from '../services/UpdateNotifications';
...
export default function HomeScreen() {
  ...
  const [searchRes, setSearchRes] = useState<{code: string; description: string; source: string}[]>([]);
    ...
    if (!isCodeSearch && results.length < 30) {
      for (const [code, advice] of Object.entries(DTC_AI_ADVICE)) {
        ...
        if ((advice.cause + ' ' + advice.advice).toLocaleLowerCase('tr').includes(lowerTR)) {
          results.push({code, description: (DTC_DESCRIPTIONS[code] || getDtcDescription(code) || code) + ' (AI)', source: 'AI'});
          seenCodes.add(code);
        }
        if (results.length >= 30) break;
      }
      ...
      <SafeAreaView edges={['top', 'bottom']} style={[styles.container, {backgroundColor: colors.bg}]}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <View style={styles.header}>
            ...
            <TouchableOpacity onPress={handleGoVehicles} style={{flexDirection: 'row', alignItems: 'center'}}>
              ...
              <View>
                ...
                <View style={{flexDirection: 'row', alignItems: 'center', gap: 8}}>
                  ...
                  {dtcScanCount > 0 && (
                    <TouchableOpacity onPress={() => navigate('errorcodes')} style={{backgroundColor: 'rgba(255,71,87,0.15)', borderRadius: 8, paddingHorizontal: 6, paddingVertical: 2}}>
                      <Text style={{color: '#ff4757', fontSize: 10, fontWeight: '800'}}>DTC {dtcScanCount}</Text>
                    </TouchableOpacity>
                  )}
                </View>
              </View>
            ...
            <TouchableOpacity onPress={() => {setShowSearch(true); setSearchQ(''); setSearchRes([]);}} style={{padding: 8}}>
              <Text style={{fontSize: 20}}>🔎</Text>
            </TouchableOpacity>
        ...
        <Modal visible={showSearch} transparent animationType="slide">
          <View style={{flex:1, backgroundColor:'rgba(0,0,0,0.85)', paddingTop:60}}>
            <View style={{flex:1, backgroundColor:colors.bg, borderTopLeftRadius:30, borderTopRightRadius:30, padding:20}}>
              <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:16}}>
                <Text style={{color:colors.text, fontSize:18, fontWeight:'900', letterSpacing:1}}>🔎 EVRENSEL ARAMA</Text>
              ...
              <TextInput
                style={[{
                  ...
                  borderRadius:12, padding:14, fontSize:16, borderWidth:1, marginBottom:10,
                  textAlign:'center', fontWeight:'800', letterSpacing:2,
                }]}
                value={searchQ}
                onChangeText={t => {setSearchQ(t); handleGlobalSearch(t);}}
              ...
              />
              {searchRes.length > 0 && (
                <Text style={{color:colors.textDim, fontSize:11, marginBottom:8}}>
                  {searchRes.length} sonuç bulundu ({searchRes.filter(r=>r.source==='VT').length} VT + {searchRes.filter(r=>r.source==='AI').length} AI)
                </Text>
              )}
              <ScrollView>
                {searchRes.map((item, idx) => (
                  <TouchableOpacity
                    key={item.code + idx}
                    onPress={() => {navigation.navigate('ErrorCodes' as any); setShowSearch(false);}}
                    onLongPress={() => {Linking.openURL(`https://www.google.com/search?q=${encodeURIComponent(item.code + ' OBD2 hata kodu')}`).catch(()=>{});}}
                    style={{backgroundColor: colors.card, borderRadius:14, padding:14, marginBottom:8, borderLeftWidth:3, borderLeftColor: getDTCCategoryColor(item.code)}}>
                    <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center'}}>
                      <Text style={{color: getDTCCategoryColor(item.code), fontSize:18, fontWeight:'900', letterSpacing:1}}>📖 {item.code}</Text>
                      <Text style={{fontSize:9, fontWeight:'700', color: item.source === 'AI' ? '#00bfff' : '#ffa502', backgroundColor:'rgba(255,255,255,0.06)', paddingHorizontal:4, paddingVertical:1, borderRadius:3}}>{item.source}</Text>
                    </View>
                    <Text style={{color:colors.textDim, fontSize:12, marginTop:4, lineHeight:16}}>{item.description}</Text>
                  </TouchableOpacity>
                ))}
                {searchRes.length === 0 && searchQ.length >= 2 && (
                  <Text style={{color:colors.textDim, textAlign:'center', marginTop:40, fontSize:14}}>Sonuç bulunamadı.</Text>
                )}
              </ScrollView>
            </View>
          </View>
