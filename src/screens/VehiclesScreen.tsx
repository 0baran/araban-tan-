...
import {
  loadVehicles, getVehicles, addVehicle, updateVehicle, deleteVehicle,
  type Vehicle, generateId,
...
export default function VehiclesScreen({onBack}: Props) {
  ...
  const openNew = () => {
    setEditId(null); setName(''); setPlate(''); setVin('');
    setBrand(''); setModel(''); setYear('');
    setShowForm(true);
  };

  const openEdit = (v: Vehicle) => {
    setEditId(v.id); setName(v.name); setPlate(v.plate || '');
    setVin(v.vin || ''); setBrand(v.brand || ''); setModel(v.model || '');
    setYear(v.year || ''); setShowForm(true);
  };
  ...
  const remove = (id: string) => {
    Alert.alert('Sil', 'Bu aracı silmek istediğinize emin misiniz?', [
      ...
      {text: 'Sil', style: 'destructive', onPress: async () => { await deleteVehicle(id); refresh(); }},
    ]);
  };
  ...
  return (
    <SafeAreaView edges={['top', 'bottom', 'left', 'right']} style={[styles.container, {backgroundColor: colors.bg}]}>
      ...
      <ScrollView contentContainerStyle={styles.list}>
        {list.length === 0 && (
          <Text style={[styles.emptyText, {color: colors.textMuted}]}>Kayıtlı araç yok. + EKLE ile ekleyin.</Text>
        )}
        {list.map(v => (
          <TouchableOpacity key={v.id} style={[styles.card, {backgroundColor: colors.card, borderColor: colors.cardBorder}]} onPress={() => openEdit(v)} onLongPress={() => remove(v.id)}>
            <View style={styles.cardHeader}>
              <Text style={[styles.cardBrand, {color: colors.text}]}>{v.brand || v.name}</Text>
              {v.plate ? <Text style={[styles.cardPlate, {color: colors.accent}]}>{v.plate}</Text> : null}
            </View>
            {v.model || v.year ? <Text style={[styles.cardDetail, {color: colors.textMuted}]}>{[v.model, v.year].filter(Boolean).join(' / ')}</Text> : null}
            {v.vin ? <Text style={[styles.cardVin, {color: colors.textMuted}]}>VIN: {v.vin}</Text> : null}
            {v.lastConnected ? <Text style={[styles.cardConnected, {color: colors.textDim}]}>Son: {v.lastConnected}</Text> : null}
          </TouchableOpacity>
        ))}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  modalTitle: {fontSize: 18, fontWeight: '900', marginBottom: 20, letterSpacing: 1},
  ...
  btn: {flex: 1, padding: 16, borderRadius: 14, alignItems: 'center', borderWidth: 1},
  btnPrimary: {},
