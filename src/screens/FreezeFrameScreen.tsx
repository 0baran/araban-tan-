...
export default function FreezeFrameScreen({onBack}: Props) {
  ...
  const rows: {label: string; value: string; color: string}[] = [
    ...
    {
      label: 'MAF',
      value: `${ff?.maf.toFixed(1) || '0.0'} g/s`,
      color: '#7bed9f',
    },
    {
      label: 'Gaz Kelebeği',
      value: `%${ff?.throttlePos || 0}`,
      color: '#70a1ff',
    },
    {label: 'MAP Basıncı', value: `${ff?.map || 0} kPa`, color: '#ff4757'},
    {
      label: 'Ateşleme Avansı',
      value: `${ff?.timingAdvance || 0}°`,
      color: '#2ed573',
    },
    {
      label: 'Kısa Dönem Yakıt',
      value: `${ff?.shortTermFuelTrim || 0}%`,
      color: '#ffafcc',
    },
    {
      label: 'Uzun Dönem Yakıt',
      value: `${ff?.longTermFuelTrim || 0}%`,
      color: '#cdb4db',
    },
    {
      label: 'Hedef H/Y Oranı',
      value: `${ff?.commandedAFR.toFixed(1) || '0.0'} λ`,
      color: '#a2d2ff',
    },
      ...
      {loading ? (
        ...
        <>
            ...
            <View style={styles.grid}>
              {rows.map(r => (
                <View key={r.label} style={styles.sensorItem}>
                  <Text style={[styles.sensorValue, {color: r.color}]}>
                    {r.value}
                  </Text>
          ...
          <View
            ...
            <Text style={[styles.cardLabel, {color: colors.textMuted}]}>
              BEKLEYEN HATA KODLARI
            </Text>
            {pending.length === 0 ? (
              <Text style={[styles.emptyText, {color: colors.textMuted}]}>
                Bekleyen hata kodu yok
              </Text>
            ) : (
              pending.map(d => (
                <View key={d.code} style={styles.pendingItem}>
                  <Text style={styles.pendingCode}>{d.code}</Text>
                  <Text style={[styles.pendingDesc, {color: colors.textDim}]}>
                    {d.description}
                  </Text>
                </View>
              ))
