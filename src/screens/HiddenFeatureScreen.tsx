  ...
  const toggleFeature = useCallback(
    async (f: HiddenFeature, turnOn: boolean) => {
      ...
      Alert.alert(
        ...
        `${f.name} ${turnOn ? 'açıldı' : 'kapatıldı'}. Yanıt: ${
          resp || '(boş)'
    ...
    <SafeAreaView
      ...
      <ScrollView contentContainerStyle={{padding: 20, paddingBottom: 60}}>
        <View
          ...
          {!vin && !vinLoading && (
            <TouchableOpacity style={styles.vinBtn} onPress={loadVIN}>
              <Text style={styles.vinBtnText}>VIN OKU</Text>
            </TouchableOpacity>
          )}
        </View>
          ...
          <View style={styles.mfrRow}>
            {allMfrs.map(m => (
              <TouchableOpacity
                ...
                style={[
                  styles.mfrBtn,
                  manufacturer === m && styles.mfrBtnActive,
                ]}
                onPress={() => selectManufacturer(m)}>
                <Text style={styles.mfrBtnLabel}>{getManufacturerIcon(m)}</Text>
                <Text
                  style={[
                    styles.mfrBtnText,
                    manufacturer === m && styles.mfrBtnTextActive,
                  ]}>
        ...
        {features.length === 0 && (
          <View
            ...
            <Text style={[styles.cardDesc, {color: colors.textDim}]}>
              Seçilen marka için henüz özellik tanımlanmamış. Aşağıdan özel
        ...
        {Object.entries(grouped).map(([category, catFeatures]) => (
          <View key={category} style={styles.featureGroup}>
            <TouchableOpacity
              ...
              <Text style={[styles.groupArrow, {color: colors.textMuted}]}>
            ...
            {expandedSections[category] &&
              catFeatures.map(f => (
                <View key={f.id} style={styles.featureCard}>
                  ...
                  <Text style={[styles.featureDesc, {color: colors.textDim}]}>
                    {f.description}
                  </Text>
                  {f.compatibility ? (
                    <Text style={styles.compatText}>
                      Uyum: {f.compatibility}
                    </Text>
                  ) : null}
                  {readResults[f.id] ? (
                  ...
                  <View style={styles.actions}>
                    ...
                    {f.writeOn ? (
                      <TouchableOpacity
                        style={[styles.actionBtn, styles.actionOn]}
                        onPress={() => toggleFeature(f, true)}
                        disabled={loading[f.id]}>
                        <Text style={[styles.actionText, {color: '#00ff7f'}]}>
                          AÇ
                        </Text>
                      </TouchableOpacity>
                    ) : null}
                    {f.writeOff ? (
                      <TouchableOpacity
                        style={[styles.actionBtn, styles.actionOff]}
                        onPress={() => toggleFeature(f, false)}
                        disabled={loading[f.id]}>
        ...
        <View
          ...
          <Text style={[styles.cardDesc, {color: colors.textDim}]}>
            CAN ID ve hex komut girerek kendi kodlamanızı gönderin
          </Text>
          ...
          <TouchableOpacity
            style={[styles.actionBtn, {alignSelf: 'center', marginTop: 10}]}
            onPress={sendCustomCommand}
            disabled={loading._custom}>
            {loading._custom ? (
              <ActivityIndicator size="small" color="#00bfff" />
            ) : (
              <Text style={[styles.actionText, {color: colors.text}]}>
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={[styles.infoText, {color: colors.textDim}]}>
            ...
            araç/ECU tüm komutları desteklemeyebilir{'\n'}•
