...
class VoiceAlertService {
  ...
  public checkAlerts(data: OBD2Data) {
    ...
    if (data.batteryVoltage > 0 && data.batteryVoltage < 11.5) {
      this.triggerAlert(
        'battery',
        'Dikkat, akü voltajı çok düşük. Elektrik arızası riski var.',
        now,
      );
    }
    ...
    if (data.coolantTemp > 105) {
      this.triggerAlert(
        'coolant',
        'Dikkat, motor harareti çok yüksek. Lütfen motoru dinlendirin.',
        now,
      );
    }
    ...
    if (data.speed > 120) {
      this.triggerAlert(
        'speed',
        'Aşırı hız uyarısı. Lütfen hız sınırlarına uyun.',
        now,
        3 * 60 * 1000,
      ); // 3 dk cooldown
    }
